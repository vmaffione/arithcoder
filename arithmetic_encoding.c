...
int arithmetic_encoding(const char* file_to_encode, const char* encoded_file)
  {
	FILE* fin;
	uint8_t symbol_8;
	unsigned int symbol;
	...
	struct BitBuffer* bbp = bit_open(encoded_file, BIT_BUFFER_WRITE);
	if (bbp == NULL)
		return -1;
	...
	for (;;)
	  {
		...
		*/
			...
			if (mask & LOW & HIGH)
				...
				/*  1000..00..  =>  0111..11
					...
					0111..10..  =>  0111..10  (1000..00)
					  */
				data = ((uint64_t)1 << (i-1)) - 1;
			bit_write(bbp, data, i);
			LOW &= ~mask;
			HIGH |= mask;
			HIGH <<= i;
			LOW <<= i;
		...
		while (mask && (tmp & mask) == 0)
		  {
			i++;
			//data >>= 1;if (LOW & mask) data |= 0x8000000000000000;
			mask >>= 1;
		  }
		//data >>= (sizeof(uint64_t) * 8 - i);
		data = LOW >> (sizeof(uint64_t) * 8 - i);
		bit_write(bbp, data, i);
		LOW <<= i;
		HIGH <<= i;
		// adaptive update
